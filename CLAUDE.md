...
## PR・マージの手順（例外なし）
  ...
  実装 → PR作成 → 報告 → けんの裁定 → マージ

- 既定は「PR作成→報告」で停止する。PRは open のまま残す。
- 裁定を経ないマージ（merge / squash merge / rebase merge / main への直push）は禁止。
  内容が正しいことは、マージしてよい理由にならない。
- けんが明示的に「マージして」と指示した場合に限り、マージを実行してよい。
  指示が曖昧なとき（「OK」「いいね」等だけのとき）は実行せず、
  「マージまで実行してよいか」を1行で確認する。
- 読者に見える変更があるPRは、PR本文に [updates] 行を入れる。
