...
# Windowsタスクスケジューラから毎日 09:00 と 18:30 に実行される想定
...
#     - beforeinfo は Actions が随時追記   → 新しい開催日の行が生えるのは
#       翌朝の第1便 JST 07:30〜07:44（直近8日の実測。幅14分・例外なし）
#   06:00 の1本だけだと補填が常に beforeinfo より先に走り、最新1日分が
#   毎日およそ22時間ぶん空のまま残る
#   （2026-07-31 に82件、2026-08-03 に221件で観測）。
#
# dailyMotorUsage.ps1 側の backfill 呼び出しは削除していない。三段構えにする:
#   09:00 … 第1便で生えたぶんを埋める（本命。空白は約1.4時間に縮む）
#   18:30 … 第1便が GitHub のスケジュール遅延(1.5〜3.8時間の実績)で
#           09:00 を越えて着地した日を拾う（回収）
#   翌 06:00 … そこまでで漏れたぶんを拾う（dailyMotorUsage 側・最終保険）
