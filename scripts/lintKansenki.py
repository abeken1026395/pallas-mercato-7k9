...
DUP_PREFS = ("北海道", "青森", "岩手", "宮城", "秋田", "山形", "福島", "茨城", "栃木", "群馬", "埼玉", "千葉",
...
def _dup_norm(sent, names):
    """数字と選手名・場名・都道府県名（支部）・級別を記号に置き換えた、文の形（2026-09-22 県と級を追加）。"""
    x = sent
    for n in sorted(names, key=len, reverse=True):
        if n:
            x = x.replace(n, "〈名〉")
    for p in sorted(DUP_PREFS, key=len, reverse=True):
        x = x.replace(p, "〈県〉")
    x = re.sub(r"[ABＡＢ][12１２]", "〈級〉", x)
