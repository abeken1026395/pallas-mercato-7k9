...
|一部保留|T5 場の文化・T7 検証01・T10 用語辞典・T12 占い（下の「裁定待ち」）|
...
- **全ページ監査の保留分**（`notes/pageAuditProgress.md` に理由）。どれも計画書の「触ってよいファイル」外か、コードから確認できなかったもの。直すなら正本のファイルを指定して裁定する
  - T5 場の文化：P5（「AI提供」「memo保留」「present=unknown」・住之江の人数）・J7 文言（「全国で唯一」「開場から70年級」）・N9（大村「日本一堅い」）。正本は `docs/data/yosouya.json`・`docs/data/fanfareBgm.json`
  - T10 用語辞典：J5（帰郷の読み「きこう」）・J6（カド・スロー勢・ダッシュ勢・深イン）。正本は `docs/data/kansenki/glossary.json`
  - T7 検証01：N4 の3表の件数差（6,701／7,173／7,254）と「386日」。`buildKensho.py` の表は固定値で元の集計がリポジトリに無い。results/ は期間388日すべてある
  - T12 占い：J14「複勝率」。`fukusho` は5〜6台の値で3着以内の割合ではない（計算元不明）ので「3連率」に言い換えていない
  - T1 見どころ：「(カドの一撃)」「(インの格が軽い)」は `scripts/build_arare.py` の文字列（T1 の対象外）。T2 の J10 のうち `docs/highlights/index.html` 側の選手名4文字切りも未変更
